# ==================== ULTIMATE SOLO LEVELING BOT — FINAL & PERFECT ====================
...
class GuideView(ui.View):
    ...
    async def beginner(self, interaction: discord.Interaction, button):
        await interaction.response.send_message(embed=discord.Embed(title="Beginner Guide", color=0x00ff00,
    ...
    async def training(self, interaction: discord.Interaction, button):
        await interaction.response.send_message(embed=discord.Embed(title="Training", color=0x5865f2,
    ...
    async def classes(self, interaction: discord.Interaction, button):
        await interaction.response.send_message(embed=discord.Embed(title="Classes", color=0xff0000,
            description="Lv80 → /jobchange\nNecromancer = shadows\nMonarch = damage"), ephemeral=True)

    @ui.button(label="All Commands", style=discord.ButtonStyle.grey)
    async def cmds(self, interaction: discord.Interaction, button):
        cmds = [c.name for c in tree.get_commands()]
        await interaction.response.send_message(embed=discord.Embed(title="Commands", description="\n".join(f"`/{c.name}`" for c in cmds), color=0x2f3136), ephemeral=True)
